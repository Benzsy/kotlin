...
abstract class WebStaticInitializersDeclarationLowering : FileLoweringPass {
    ...
    private fun processDeclarationContainer(container: IrClass) {
        ...
        if (container.isNonCompanionObject && container.isInitializersFreeClass()) return
        ...
        fun MutableList<IrStatement>.fixObjectGetInstanceFunction(declaration: IrClass) {
            declaration.objectGetInstanceFunction?.let { getInstance ->
                ...
                body.statements.let { statements ->
                    ...
                    add(context.createIrBuilder(getInstance.symbol).irCallConstructor(primaryConstructor.symbol, emptyList()))
                }
            }
        }
